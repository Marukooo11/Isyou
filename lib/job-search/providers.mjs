...
export class OpenAIWebSearchProvider {
  constructor({ apiKey = process.env.OPENAI_API_KEY, model = process.env.OPENAI_SEARCH_MODEL || "gpt-5-mini", baseUrl = process.env.OPENAI_BASE_URL || "https://api.openai.com/v1", proxyUrl = process.env.JOB_SEARCH_PROXY_URL, maxOutputTokens = Number(process.env.OPENAI_SEARCH_MAX_OUTPUT_TOKENS || 1800), fetchImpl = fetch, timeoutMs = 45000 } = {}) {
    ...
    this.name = /(^|\.)deepseek\.com$/i.test(new URL(this.baseUrl).hostname) ? "deepseek_web_search" : "openai_web_search";
    this.displayName = this.name === "deepseek_web_search" ? "DeepSeek Web Search" : "OpenAI Web Search";
  ...
  async search(query, { limit = 10, direction = null } = {}) {
    ...
    try {
      ...
      throw new PipelineError("SEARCH_PROVIDER_NETWORK_ERROR", `无法连接 ${this.displayName}（${reason}）。`, 502, { provider: this.name, reason });
    }
    if (!response.ok) {
      let upstream = {};
      ...
      const message = response.status === 402
        ? `${this.displayName} 账户余额不足，请充值或更换有余额的密钥。`
        : `${this.displayName} 返回 ${response.status}（${reason}）。`;
      throw new PipelineError("SEARCH_PROVIDER_ERROR", message, 502, { provider: this.name, status: response.status, reason });
    }
